...
function renderArray() {
  ...
  for (let i = 0; i < randomLetters.length; i++) {
    ...
    el.textContent = randomLetters[i];
...
function selectGoodLetters() {
  ...
  for (let i = 0; i < 4; i++) {
    ...
    goodLetters[randomPosition + i] = randomLetters[randomPosition + i - 1];
  }
}
...
function check() {
  ...
  selectedElements.forEach((el) =>
    selectedLetters.push(el.textContent)
  );

  for (let i = 0; i < 4; i++) {
    if (selectedLetters[i] == goodLetters[lettersToFind[i]]) {
      gameWin();
      return;
    }
  }
